...
def load_img(fname, size=None, flip=False):
    ...
    try:
        ...
        print(f"[ERR] {path}: {e}"); return None

class Assets:
    def __init__(self):
        self.bg=None; self.dragons={}; self.bosses={}
    def load(self,W,H):
    ...
    def dragon(self,e): return self.dragons.get(e)
    def boss(self,e):   return self.bosses.get(e)
...
class Pt:
    def __init__(self,x,y,col,vx,vy,sz,life):
        ...
        self.vx=vx;self.vy=vy;self.sz=sz;self.life=life;self.ml=life
    def update(self):
...
class Particles:
    ...
    def emit(self,x,y,elem,n=25):
        ...
        for _ in range(n):
            ang=random.uniform(0,math.pi*2); spd=random.uniform(2,8)
    ...
    def firework(self,x,y):
        ...
        for _ in range(18):
            ang=random.uniform(0,math.pi*2); spd=random.uniform(3,9)
            self.L.append(Pt(x,y,col,math.cos(ang)*spd,math.sin(ang)*spd,
                             random.randint(3,8),random.randint(28,52)))
    def update(self):
        self.L=[p for p in self.L if p.life>0]
        for p in self.L: p.update()
    def draw(self,s):
        for p in self.L: p.draw(s)

# ==========================================
# SMOOTH FLOAT
# ==========================================
class SF:
    def __init__(self,v=0.0,spd=0.1): self.c=self.t=float(v); self.s=spd
    def set(self,v): self.t=float(v)
    def snap(self,v): self.c=self.t=float(v)
    def update(self): self.c+=(self.t-self.c)*self.s
...
# ==========================================
class Anim:
    IDLE="i"; ENTER="e"; ATK="a"; DIE="d"; RETREAT="r"

    def __init__(self,hx,hy,bench_x,bench_y):
        self.hx=hx;self.hy=hy
        self.bx=bench_x;self.by=bench_y
        self.x=float(hx);self.y=float(hy)
        self.alpha=255;self.scale=1.0;self.angle=0.0
        self.state=self.IDLE;self.t=0;self.cb=None

    def enter(self,cb=None):
        self.x=float(self.bx);self.y=float(self.by)
        self.alpha=0;self.scale=0.5
        self.state=self.ENTER;self.t=0;self.cb=cb

    def attack(self,tx,ty,cb=None):
        self.tx=tx;self.ty=ty
        self.state=self.ATK;self.t=0;self.cb=cb

    def retreat(self,cb=None):
        self.state=self.RETREAT;self.t=0;self.cb=cb

    def die(self,cb=None):
        self.state=self.DIE;self.t=0;self.cb=cb
    ...
    def _done(self):
        if self.cb:
            c=self.cb;self.cb=None;c()

    def update(self):
        if self.state==self.IDLE:
            self.x=float(self.hx);self.y=float(self.hy)
            self.scale=1.0;self.angle=0;self.alpha=255
            ...
            if p>=1.0:
                ...
                self.state=self.IDLE;self._done()
            ...
            if self.t<=25:
                ...
                if p>=1.0:
                    self.x=float(self.hx);self.y=float(self.hy)
                    self.scale=1.0;self.state=self.IDLE
            ...
            if p>=1.0:
                self.x=float(self.bx);self.y=float(self.by)
                self.scale=0.5;self.state=self.IDLE;self._done()
    ...
    def draw(self,surf,img,size):
        if img is None or self.alpha<=0: return
        sw=max(1,int(size[0]*self.scale))
        sh=max(1,int(size[1]*self.scale))
        s=pygame.transform.smoothscale(img,(sw,sh))
        if abs(self.angle)>0.3: s=pygame.transform.rotate(s,self.angle)
        if self.alpha<255: s.set_alpha(self.alpha)
...
class Skill:
    def __init__(self, n, stype, dmg, acc, cd, heal=0):
        ...
        self.stype    = stype
        self.damage   = dmg
        self.accuracy = acc      # % trúng (và % hồi máu với Ultim)
        self.max_cd   = cd
        self.cd       = 0
        self.heal     = heal     # HP hồi cho bản thân khi trúng (Ultim)
...
class GS:
    ...
    def winner(self):
        if not self.p1.has_alive(): return self.p2.name
        if not self.p2.has_alive(): return self.p1.name
        return None
    ...
    def actions(self):
        ...
        - Dùng chiêu (CD = 0)
        - Đổi rồng tự do (chỉ cần alive, không bị lock)
        """
        acts = []
        cp   = self.cur()
        ad   = cp.active()

        # Dùng chiêu trên con đang đánh
        for si, s in enumerate(ad.skills):
            if s.ready():
                acts.append({"t": "skill", "si": si})

        # Đổi rồng tự do, chỉ cần alive và không phải con đang đánh
...
def do_act(gs, act, real=False, force=None):
    ...
    # ---------------------------------------------------
    # Xác định xem có trúng không
    # force=True  → luôn trúng (dùng trong AI simulation)
    # force=False → luôn miss (dùng cho Expectimax chance node)
    # real=True   → roll random thật
    # ---------------------------------------------------
    if force is not None:
        ...
        # Simulation mặc định: tính theo xác suất kỳ vọng (coi như trúng)
        hit = True
    ...
    info = {
        ...
        "mult":     1.0,
        "dmg":      0,
        "heal":     0,       # HP thực sự hồi được
        "ae":       ad.element,
        "de":       dd.element,
        "def_died": False,
        "stype":    sk.stype,
        "atk_name": ad.name,   # tên rồng tấn công (để hiển thị heal popup)
    }

    if hit:
        if sk.stype == "normal":
            # Slash: 50 dmg cố định, không tính element
            dmg        = 50
            info["mult"] = 1.0
        else:
            # Power / Ultim: tính element multiplier
        ...
        # Hồi máu cho bản thân nếu skill có heal (Ultim)
        if sk.heal > 0:
            ad.heal(sk.heal)
            info["heal"] = sk.heal

    # ---------------------------------------------------
    # CD: +1 để bù lần giảm ngay bên dưới
    # max_cd=4 → sau 4 lượt của chủ = turn thứ 5 mới ready
    # max_cd=7 → sau 7 lượt của chủ = turn thứ 8 mới ready
    # ---------------------------------------------------
    sk.cd = sk.max_cd + 1   # gán trước

    # Chỉ giảm CD của người vừa đánh (owner-turn based)
    for d2 in atk.dragons:
        for s2 in d2.skills:
            if s2.cd > 0:
                s2.cd -= 1  # ngay lập tức trừ 1 → sk.cd = max_cd
...
def evaluate(gs, p1p):
    ...
    sc  = (sum(d.hp for d in my.dragons if d.alive)
         - sum(d.hp for d in op.dragons if d.alive))
    sc += (len(my.alive_idx()) - len(op.alive_idx())) * 1000

    md, od = my.active(), op.active()
    if get_mult(md.element, od.element) == 2.0: sc += 100
    if get_mult(od.element, md.element) == 2.0: sc -= 50
    if md.skills[1].ready(): sc += 30
    if md.skills[2].ready(): sc += 50
    return sc
...
class Minimax:
    ...
    def run(gs, depth=2):
        is_p1 = (gs.turn % 2 == 0)
        _, act = Minimax._mm(gs, depth, True, is_p1)
        return act

    @staticmethod
    def _mm(st, d, mx, p1):
        if d == 0 or st.over(): return evaluate(st, p1), None
        acts = st.actions()
        if not acts: return evaluate(st, p1), None
        best = acts[0]
        be   = float('-inf') if mx else float('inf')
        for a in acts:
            sim = st.clone(); do_act(sim, a, force=True)
            e, _ = Minimax._mm(sim, d-1, not mx, p1)
            if (mx and e > be) or (not mx and e < be):
                be, best = e, a
        return be, best
...
class AlphaBeta:
    ...
    def run(gs, depth=2):
        is_p1 = (gs.turn % 2 == 0)
        _, act = AlphaBeta._ab(gs, depth, float('-inf'), float('inf'), True, is_p1)
        return act

    @staticmethod
    def _ab(st, d, al, bt, mx, p1):
        if d == 0 or st.over(): return evaluate(st, p1), None
        acts = st.actions()
        if not acts: return evaluate(st, p1), None
        best = acts[0]
        ...
        for a in acts:
            sim = st.clone(); do_act(sim, a, force=True)
            e, _ = AlphaBeta._ab(sim, d-1, al, bt, not mx, p1)
            if (mx and e > be) or (not mx and e < be):
                be, best = e, a
            if mx: al = max(al, be)
            else:  bt = min(bt, be)
...
class Expectimax:
    ...
    def run(gs, depth=2):
        is_p1 = (gs.turn % 2 == 0)
        _, act = Expectimax._ex(gs, depth, True, is_p1)
        return act
    ...
    def _ex(st, d, mx, p1):
        ...
        be   = float('-inf') if mx else float('inf')
        for a in acts:
            e = Expectimax._chance(st, a, d, not mx, p1)
            if (mx and e > be) or (not mx and e < be):
                be, best = e, a
        return be, best

    @staticmethod
    def _chance(st, action, d, next_mx, p1):
        cp  = st.cur()
        idx = action["di"] if action["t"] == "sw" else cp.ai
        acc = cp.dragons[idx].skills[action["si"]].accuracy / 100.0
        sh = st.clone(); do_act(sh, action, force=True)
        eh, _ = Expectimax._ex(sh, d-1, next_mx, p1)
        em = 0.0
        if acc < 1.0:
            sm = st.clone(); do_act(sm, action, force=False)
            em, _ = Expectimax._ex(sm, d-1, next_mx, p1)
        return acc * eh + (1.0 - acc) * em


class AI:
    @staticmethod
    def act(algo, gs, depth=2):
        if algo == "Minimax":    return Minimax.run(gs, depth)
        if algo == "AlphaBeta":  return AlphaBeta.run(gs, depth)
        if algo == "Expectimax": return Expectimax.run(gs, depth)
...
def grad_rect(surf, c1, c2, rect, vert=True):
    ...
    for i in range(n):
        t = i / max(1, n-1); c = lerp_c(c1, c2, t)
        if vert: pygame.draw.line(surf, c, (rect.x, rect.y+i), (rect.x+rect.w, rect.y+i))
        else:    pygame.draw.line(surf, c, (rect.x+i, rect.y), (rect.x+i, rect.y+rect.h))
...
class Stage6Boss:
    def __init__(self, screen, stage_manager=None):
        ...
        self.shake_t = 0; self.shake_a = 0
        self.flash_c = (255,255,255); self.flash_v = 0
        ...
        self.last_heal_info = None   # <<< popup hồi máu riêng
    ...
    def reset(self):
        ...
        for i in range(6): self.hp_sm[i].snap(1.0)
    ...
    def _sync_positions(self):
        ...
        for i in range(3):
            a1 = self.anims[("p1", i)]
            if i == self.gs.p1.ai:
                a1.hx = LO["p1fx"]; a1.hy = LO["fy"]
                a1.x = float(a1.hx); a1.y = float(a1.hy)
            else:
                cx, cy = LO["p1_cards"][i]
                a1.hx = a1.bx = cx; a1.hy = a1.by = cy
                a1.x = float(cx); a1.y = float(cy)
            a1.alpha = 255; a1.scale = 1.0; a1.state = Anim.IDLE

            a2 = self.anims[("p2", i)]
            if i == self.gs.p2.ai:
                a2.hx = LO["p2fx"]; a2.hy = LO["fy"]
                a2.x = float(a2.hx); a2.y = float(a2.hy)
            else:
                cx, cy = LO["p2_cards"][i]
                a2.hx = a2.bx = cx; a2.hy = a2.by = cy
                a2.x = float(cx); a2.y = float(cy)
            a2.alpha = 255; a2.scale = 1.0; a2.state = Anim.IDLE
        ...
        for ev in events:
            if ev.type == pygame.MOUSEBUTTONDOWN:
                ...
                if self.b_back.collidepoint(mx, my):
                    if self.sm: self.sm.change_stage("stage_select")
                    return

                changed = False
                for algo, rect in self.b_algos_p1.items():
                    if rect.collidepoint(mx, my):
                        self.algo_p1 = algo; changed = True
                for algo, rect in self.b_algos_p2.items():
                    if rect.collidepoint(mx, my):
                        self.algo_p2 = algo; changed = True
                if self.b_first_p1.collidepoint(mx, my):
                    self.first_turn = 0; changed = True
                if self.b_first_p2.collidepoint(mx, my):
                    self.first_turn = 1; changed = True
                if changed:
                    self.reset(); return
    ...
    def _step(self):
        if self.locked or self.gs.over() or self.switch_timer > 0: return
        act = AI.act(self._get_algo(), self.gs, depth=2)
        if not act: return
        self.locked = True
        self._animate(act)
    ...
    def _do_atk(self, aside, ai, dside, di, act):
        ...
        def on_hit():
            real_act = dict(act)
            if real_act["t"] == "sw": real_act["t"] = "skill"
            info = do_act(self.gs, real_act, real=True)

            # Bên tấn công (sau khi gs đã update)
            p_atk     = self.gs.p1 if aside == "p1" else self.gs.p2
            atk_dragon = p_atk.dragons[ai]   # rồng vừa tấn công (dùng index cũ)
            ...
            if not info["hit"]:
                ...
                if info["stype"] == "normal":
                    ...
                    m = info["mult"]
                    if m >= 2.0:   dmg_str = f"{int(info['dmg'])} dmg (STRONG!)"
                    elif m <= 0.5: dmg_str = f"{int(info['dmg'])} dmg (weak)"
                    else:          dmg_str = f"{int(info['dmg'])} dmg"
                # Thêm thông tin heal vào log nếu có
            ...
            # ---- Heal popup (hiện trên rồng tấn công) ----
            if info.get("heal", 0) > 0:
                ...
                # Hiệu ứng hạt xanh lá trên rồng tấn công
    ...
    def update(self):
        ...
        if self.flash_v > 0: self.flash_v = max(0, self.flash_v - 8)
        self.ptc.update()
        for s in self.hp_sm: s.update()
        for a in self.anims.values(): a.update()
    ...
    def T(self, txt, font, col, x, y, cx=False, sh=False):
        if sh:
            s = font.render(txt, True, (0,0,0)); r = s.get_rect()
            if cx: r.center  = (x+2, y+2)
            else:  r.topleft = (x+2, y+2)
            self.screen.blit(s, r)
        s = font.render(txt, True, col); r = s.get_rect()
        if cx: r.center  = (x, y)
        else:  r.topleft = (x, y)
        self.screen.blit(s, r)

    def btn(self, rect, label, active=False, danger=False, accent=None, font=None):
        if font is None: font = self.f13
        mx, my = pygame.mouse.get_pos(); hov = rect.collidepoint(mx, my)
        if   active:         bg = accent or (60,120,210); bd = (255,255,255)
        elif hov and danger: bg = (140,40,40);             bd = (200,80,80)
        elif hov:            bg = (72,80,96);              bd = (140,140,140)
        elif danger:         bg = (78,24,24);              bd = (130,50,50)
        else:                bg = (42,46,58);              bd = (65,70,88)
    ...
    def draw_panel(self):
        ...
        c_nobita = (100,180,255); c_boss = (255,110,110)
        ...
        # ---- Skill rules legend (đã cập nhật) ----
        ly = dy + 8
    ...
    def draw_card(self, dragon, side, idx, hpi):
        ...
        if fs.get_width() <= fw: self.screen.blit(fs, (fx, ly))
        ...
        if sk_count > 0:
            sw2 = max(1, (fw-(sk_count-1)*2)//sk_count); sh2 = 13
        ...
        if not dragon.alive:
            ...
            ov.fill((0,0,0,100)); self.screen.blit(ov, rect.topleft)
    ...
    def draw_fighter(self, side, idx):
        ...
        sk_w  = 38; total = len(d.skills)*(sk_w+2)-2
    ...
    def draw_damage_popup(self):
        """Popup sát thương (màu đỏ/cam) và hồi máu (màu xanh lá) riêng biệt"""
        # --- Damage popup ---
        if self.last_dmg_info:
            ...
            if age > 70:
                ...
                if not info["hit"]:
                    txt = "MISS"; col = (180,100,255); font = self.f32
                else:
                    dmg_val = str(int(info["dmg"]))
                    if info["stype"] == "normal":
                        txt = dmg_val; col = (255,165,50); font = self.f22
                    elif info["mult"] >= 2.0:
                        txt = f"STRONG\n{dmg_val}"; col = (255,60,60);   font = self.f22
                    elif info["mult"] <= 0.5:
                        txt = f"weak\n{dmg_val}";   col = (255,255,100); font = self.f22
                    else:
                        txt = dmg_val; col = (255,165,50); font = self.f22
        ...
        if self.last_heal_info:
            ...
            if age > 70:
                ...
                alpha = max(0, 255 - age*3)
                fy2   = info["y"] - age * 1.0   # bay lên chậm hơn dmg
                txt   = f"+{info['heal']} HP"
                col   = (80, 255, 140)
                font  = self.f22
    ...
    def draw_badges(self, ox, oy):
        LO  = self.LO
        p1  = self.gs.p1; al1 = len(p1.alive_idx())
        ...
        p2  = self.gs.p2; al2 = len(p2.alive_idx())
        bx2 = self.W - CCW + 4 + ox
    ...
    def draw_log(self, ox, oy):
        LO = self.LO
        lw = min(LO["arena_w"]-20, 560); lh = 40
    ...
    def draw_winner(self):
        ...
        bw,bh = 520,210
        bx = self.W//2-bw//2; by = self.H//2-bh//2
