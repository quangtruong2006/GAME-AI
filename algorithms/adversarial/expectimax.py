# GAME-AI/algorithms/adversarial/expectimax.py

def expectimax(gamestate, depth, is_maximizing, is_p1_perspective):
    ...
    for action in actions:
        score = chance_node(gamestate, action, depth, not is_maximizing, is_p1_perspective)
...
def chance_node(st, action, depth, next_is_max, is_p1):
    cp = st.cur()
    idx = action["di"] if action["t"] == "sw" else cp.ai
    acc = cp.dragons[idx].skills[action["si"]].accuracy / 100.0

    sh = st.clone()
    execute_action(sh, action, force_hit=True)
    eh, _ = expectimax(sh, depth - 1, next_is_max, is_p1)

    em = 0.0
    if acc < 1.0:
        sm = st.clone()
        execute_action(sm, action, force_hit=False)
        em, _ = expectimax(sm, depth - 1, next_is_max, is_p1)
...
def execute_action(gs, act, force_hit=True):
    atk = gs.cur()
    dfn = gs.opp()
